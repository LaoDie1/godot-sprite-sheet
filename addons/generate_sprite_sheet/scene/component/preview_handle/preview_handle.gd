...
func _on_resize_pressed():
	...
	if _size.x <= 0 or _size.y <= 0:
		GenerateSpriteSheetMain.show_message("大小不能为 0！")
		return
...
func _on_rescale_pressed():
	...
	if scale_v.x <= 0 or scale_v.y <= 0:
		GenerateSpriteSheetMain.show_message("缩放不能为 0！")
		return
...
func _on_cut_btn_pressed():
	...
	if rect.size.x <= 0 or rect.size.y <= 0:
		GenerateSpriteSheetMain.show_message("剪切图像大小不能为 0！")
		return
...
func _on_rot_pressed():
	_emit_handle(func(texture: Texture2D):
		# 旋转图片
		var new_image = GenerateSpriteSheetUtil.create_image_from(texture.get_image())
		...
		if r != 0:
			...
			GenerateSpriteSheetMain.show_message("旋转角度不能为 0！")
			return texture
	)
