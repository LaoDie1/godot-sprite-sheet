...
## 预览图像容器
@tool
class_name SpriteSheet_PreviewContainer
...
# 切分的表格
@onready var preview_split_grid := %preview_split_grid as SpriteSheet_GridRect
# 边界边框
...
func get_selected_texture_list() -> Array[ImageTexture]:
	...
	for coordinate in preview_split_grid.get_selected_coordinate_list():
		...
		new_texture = SpriteSheetUtil.create_texture_by_rect(texture, rect)
...
func _on_preview_grid_double_clicked(pos: Vector2i, coordinate: Vector2i):
	...
	if preview_rect.texture:
		...
		if rect.size == Vector2i.ZERO:
			print("[ SpriteSheet ] 没有表格大小")
			return
		
		# 创建这个区域的图片
		var texture : Texture2D = preview_rect.texture
		var new_texture = SpriteSheetUtil.create_texture_by_rect(texture, rect)
