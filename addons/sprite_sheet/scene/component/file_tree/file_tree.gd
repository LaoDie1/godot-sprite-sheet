...
#============================================================
@tool
class_name SpriteSheet_FileTree
...
func _get_config_data() -> Dictionary:
	return SpriteSheetUtil.get_config_data("file_tree")
...
func _ready():
	...
	if dir and DirAccess.dir_exists_absolute(dir):
		update_tree(dir, SpriteSheetUtil.get_texture_filter())
					...
					get_selected_items() 
						.map(func(item: TreeItem): 
								...
								if FileAccess.file_exists(d["path"]):
									return SpriteSheetUtil.load_image(d['path'])
								, 
						)
...
func _update_tree_item(parent_directory: String, parent_item: TreeItem):
	# 扫描目录
	for sub_dir in SpriteSheetUtil.scan_directory(parent_directory):
	...
	# 扫描目录下的所有文件
	for file in SpriteSheetUtil.scan_file(parent_directory):
...
func _on_tree_dragged(item: TreeItem, data_ref: Dictionary):
	...
	data_ref['type'] = SpriteSheetUtil.DragType.FileTree 
